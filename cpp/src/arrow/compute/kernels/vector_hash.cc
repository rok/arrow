  ...
  Status Append(const ArrayData& arr) override {
    if (!dictionary_) {
      ...
      ARROW_ASSIGN_OR_RAISE(
          auto out_dict_array,
          arrow::internal::checked_cast<const DictionaryArray&>(*in_dict_array)
              .Transpose(out_dict_type, out_dict, transpose));
      return indices_kernel_->Append(*out_dict_array->data().get());
    }
...
const FunctionDoc unique_doc(
    ...
    ("Return an array with distinct values. Nulls in the input are ignored."), {"array"});
