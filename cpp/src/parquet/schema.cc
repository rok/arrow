...
void SchemaDescriptor::BuildTree(const NodePtr& node, int16_t max_def_level,
  ...
  if (node->is_group()) {
    ...
    // Determine the nearest VECTOR-repeated ancestor (or this leaf itself) so
    // the resulting ColumnDescriptor can report a single effective vector
    // length regardless of where the VECTOR sits in the leaf's ancestry.
