...
class PARQUET_EXPORT ColumnDescriptor {
 public:
  ...
  /// \brief The fixed number of leaf values per parent record contributed by
  /// this column, computed as the product of all VECTOR-repeated ancestors
  /// (including the leaf itself if it is VECTOR-repeated). Returns -1 when the
  /// column is not part of a VECTOR-repeated subtree.
  ...
  /// (the leaf or some ancestor has Repetition::VECTOR).
