...
Result<std::shared_ptr<ChunkedArray>> FixedSizeListToFixedSizeBinary(
    const ChunkedArray& data, int64_t offset, int64_t size, MemoryPool* pool) {
  ...
  for (const auto& chunk : sliced->chunks()) {
    ...
    const int32_t list_byte_width = list_type.list_size() * value_byte_width;
    const int64_t data_size = chunk->length() * list_byte_width;

    auto values = list_array.values();
    if (values->null_count() != 0) {
      return Status::Invalid(
          "Cannot write FIXED_SIZE_LIST as FIXED_LEN_BYTE_ARRAY when elements are null");
    }
    ...
    auto storage_type = ::arrow::fixed_size_binary(list_byte_width);
    auto array_data =
 ...
 public:
  ...
  Status WriteColumnChunk(const std::shared_ptr<ChunkedArray>& data, int64_t offset,
    ...
    if (arrow_properties_->engine_version() == ArrowWriterProperties::V2 ||
      ...
      const auto* parquet_node =
          writer_->schema()->Column(column_index)->schema_node().get();
      if (parquet_node->logical_type()->is_fixed_size_list()) {
        ARROW_ASSIGN_OR_RAISE(
            data_to_write, FixedSizeListToFixedSizeBinary(
                               *data, offset, size, column_write_context_.memory_pool));
        offset = 0;
      }
      ARROW_ASSIGN_OR_RAISE(
  ...
  Status WriteRecordBatch(const RecordBatch& batch) override {
    ...
    auto WriteBatch = [&](int64_t offset, int64_t size) {
      ...
      for (int i = 0; i < batch.num_columns(); i++) {
        ...
        const auto* parquet_node =
            writer_->schema()->Column(column_index_start)->schema_node().get();
        if (parquet_node->logical_type()->is_fixed_size_list()) {
