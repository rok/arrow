...
| Platform | S3 | GCS | Azure | Configured in |
|----------|----|----|---------------|
| macOS (ARM64, x86_64) | ON | ON | ON | `dev/tasks/r/github.packages.yml` |
| Windows | ON | ON | OFF | `ci/scripts/PKGBUILD` |
| Linux (x86_64) | ON | ON | ON | `compose.yaml` (`ubuntu-cpp-static`) |
...
Azure is always set to OFF for Windows because of a low-level incompatibility
with MinGW. The `azure-identity-cpp` SDK for Azure relies on the Windows
Implementation Library (WIL), and this lacks stable support for MinGW.
